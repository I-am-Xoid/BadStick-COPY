...
namespace Xbox_360_BadUpdate_USB_Tool
{
    ...
    public partial class Form1 : Form
    {
        ...
        {
            // Check if community notice has been dismissed
            string settingsPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "BadStick");
            string noticeFile = Path.Combine(settingsPath, "community_notice_dismissed.flag");
            
            if (File.Exists(noticeFile))
            {
                return; // Don't show notice if already dismissed
            }
                ...
                using (var http = new HttpClient())
                {
                    ...
                    if (state == "true")
                    {
                        ...
                        var result = MessageBox.Show(
                            messageText + "\n\nWould you like to see community notices in the future?",
                            "Community Notice", 
                        ...
                        if (result == DialogResult.No)
                        {
                            try
                            {
                                ...
                                File.WriteAllText(noticeFile, DateTime.Now.ToString());
                            }
                            catch
                            {
